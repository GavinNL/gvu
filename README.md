...
**How it works**: Once set up, the DescriptorSetManger allows you to allocate descriptorSets by providing a DescriptorSetLayout. You allocate a descriptor set by providing this manager with a DescriptorSetLayoutInfo. This struct contains how many descriptors of each time exist in that particular layout. Internally, the manager keeps a list of DescriptorPools of set size for each layout. When you go to allocate a descriptor set with a particular layout, it will allocate from one of the pools. If the pool is empty, it will create a new pool to allocate from. 

When you are done with the DescriptorSet, you *return* it to the manager, when all all descriptor sets from a particular pool have been returned, it will reset the DescriptorPool.
